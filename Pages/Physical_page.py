...
role_multiselect = st.multiselect(
    "Select a role group",
    roles,
    key='physical_roles_widget'
)

# Third widget
possession_radio = st.radio(
    "Select a specific third of the peach",
    possession,
    horizontal=True,
    key='physical_third_widget'
)
...
grouping_radio = st.radio(
    ...
    group_by,
    horizontal=True,
    key='physical_grouping_widget'
)

if "physical_api_params" not in st.session_state:
    st.session_state.physical_query_params = None  # Stores the last query params
if "physical_api_response" not in st.session_state:
    st.session_state.physical_api_response = None  # Stores the last data retrieved
...
if st.button("Retrieve data"):
    try:
        ...
        set_params = {
            ...
            for key, value in {
                ...
                'possession': possession_radio if possession_radio else None,
        ...
        st.session_state.physical_query_params = set_params
        # Get the data
        response = get_physical_data(set_params)
        # Assign to session state variable
        st.session_state.physical_api_response = response
    except:
    ...
    if 'group_by' in set_params:
        st.session_state.physical_current_grouping = set_params['group_by']
    else:
        st.session_state.physical_current_grouping = None


if st.session_state.physical_api_response:
    # Display 'raw' data
    ...
    response_df = pd.DataFrame(st.session_state.physical_api_response)
    if display_selection == 'Table':
        st.dataframe(response_df)
    else:
        st.write(st.session_state.physical_api_response)
    ...
    selectbox_options = selectbox_options[selectbox_options.index('quality_check')+1:selectbox_options.index('player_name')]
    index_default_x = 1
    index_default_y = 4
    ...
    if st.session_state.physical_current_grouping == 'player' or st.session_state.physical_current_grouping is None:
        fig = scatter_chart_off_ball_runs(x_metric, y_metric, 'short_name', response_df, )
        st.pyplot(fig)
    elif st.session_state.physical_current_grouping == 'team':
